...
kernels, the compiler can combine them into a smaller number of kernels.
...
The mathematical function did not change.
...
A custom implementation may be mathematically correct and may run on the GPU,
but a library implementation can still be significantly faster because it has
...
median time in this experiment:
...
This is an important result because it shows that writing CUDA manually does
not automatically create a dramatically faster implementation.
...
Initially, the custom implementations were compared with PyTorch's default
GELU implementation and showed a maximum absolute error of approximately:
...
After using the same approximation for the PyTorch reference:
...
lowest median execution time.

However, the difference between optimized implementations was relatively
...
The custom eager PyTorch implementation was approximately:
...
slower than the fastest implementation in that benchmark.
...
The two implementations differ not only in hardware, but also in execution
model, parallelism, software overhead, and kernel organization.

The result therefore demonstrates the impact of both hardware acceleration and
...
These operations are applied across large tensors, many layers, many tokens,
and often many GPUs.
...
The same mathematical function can therefore produce very different execution
times depending on how it moves through this stack.

Key Takeaways
A GPU provides massive parallel execution capability, but software must
expose and organize that parallelism effectively.
Moving a computation to the GPU does not automatically make it efficient.
Kernel-launch overhead is especially important for small or fragmented
operations.
Kernel fusion can reduce both launch overhead and intermediate memory
traffic.
torch.compile can significantly improve performance without changing the
mathematical computation or the hardware.
Optimized PyTorch built-in kernels can outperform simple custom low-level
implementations.
CUDA provides explicit low-level control, but writing CUDA does not
automatically guarantee the fastest kernel.
Triton provides a useful middle ground between ordinary PyTorch and native
CUDA development.
Performance rankings can change with workload size.
Correctness must be verified before comparing performance.
GPU timing requires synchronization, warm-up, repeated measurements, and
appropriate timing tools.
Data movement and memory behavior can be as important as arithmetic
throughput.
Hardware architecture and software execution strategy must be considered
